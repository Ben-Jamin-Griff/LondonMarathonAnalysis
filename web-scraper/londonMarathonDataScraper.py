    ...
    if matched:
        try:
            ...
            if year > 2012:
                ...
                split5k = soupLink.select('tr.f-time_01')[0].select('td.time')[0].getText()
                splits.append(split5k)
                split10k = soupLink.select('tr.f-time_02')[0].select('td.time')[0].getText()
                splits.append(split10k)
                split15k = soupLink.select('tr.f-time_03')[0].select('td.time')[0].getText()
                splits.append(split15k)
                split20k = soupLink.select('tr.f-time_04')[0].select('td.time')[0].getText()
                splits.append(split20k)
                splitHalf = soupLink.select('tr.f-time_05')[0].select('td.time')[0].getText()
                splits.append(splitHalf)
                split25k = soupLink.select('tr.f-time_06')[0].select('td.time')[0].getText()
                splits.append(split25k)
                split30k = soupLink.select('tr.f-time_07')[0].select('td.time')[0].getText()
                splits.append(split30k)
                split35k = soupLink.select('tr.f-time_08')[0].select('td.time')[0].getText()
                splits.append(split35k)
                split40k = soupLink.select('tr.f-time_09')[0].select('td.time')[0].getText()
                splits.append(split40k)
                splitFinish = soupLink.select('tr.f-time_finish_netto')[0].select('td.f-time_finish_netto')[0].getText()
                splits.append(splitFinish)
        except:
            splits.append('-')
            splits.append('-')
...
while flag == 0 and year > 2010: #2011 is min year
    ...
    while flag == 0:
        try:
            ...
            if page == 1:
                time.sleep(1)
                res = requests.get('https://results.virginmoneylondonmarathon.com/' + str(year) + '/?page=' + str(page) + '&event=MASS&pid=search')
    ...
    page = 1
    flag = 0
    with open('scrape_' + str(year+1) + '_MASS.pkl', 'wb') as handle:
        pickle.dump(data, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Collecting elite data
year = 2019
yearCounter = 0
page = 1
flag = 0
while flag == 0 and year > 2012: #2013 is min year
    print(year)
    while flag == 0:
        try:
            print(page)
